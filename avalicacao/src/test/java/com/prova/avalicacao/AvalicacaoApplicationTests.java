package com.prova.avalicacao;

import com.prova.avalicacao.repository.UserRepository;
import com.prova.avalicacao.service.JwtService;
...
class AuthIntegrationTests {
    ...
    void setup() {
        userRepository.findByUsername("admin").ifPresentOrElse(
                user -> { /* Já existe */ },
                () -> {
                    com.prova.avalicacao.model.User admin = new com.prova.avalicacao.model.User(null, "admin", passwordEncoder.encode("123456"), "ADMIN");
        ...
        userRepository.findByUsername("user").ifPresentOrElse(
                user -> { /* Já existe */ },
                () -> {
                    com.prova.avalicacao.model.User regularUser = new com.prova.avalicacao.model.User(null, "user", passwordEncoder.encode("password"), "USER");
...
void testLoginSuccess() throws Exception {
    String token = mockMvc.perform(post("/auth/login")
           ...
           .andExpect(content().string(notNullValue())) // Verifica se o conteúdo não é nulo
           .andReturn().getResponse().getContentAsString(); // Armazena o token retornado
